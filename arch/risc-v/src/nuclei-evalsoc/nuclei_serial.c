/****************************************************************************
 * arch/risc-v/src/nuclei-evalsoc/nuclei_serial.c
 *
...
#  if defined(CONFIG_NUCLEI_EVALSOC_UART0)
#    define TTYS0_DEV       g_uart0port     /* UART0 is ttyS0 */
#    undef  TTYS1_DEV                       /* No ttyS1 */
#    define SERIAL_CONSOLE  1
#  else
...
{
  .uartbase  = NUCLEI_EVALSOC_UART0_BASE,
  .baud      = CONFIG_UART0_BAUD,
  .irq       = NUCLEI_EVALSOC_IRQ_UART0,
  .lock      = SP_UNLOCKED
};
...
static int up_attach(struct uart_dev_s *dev)
{
  ...
  //up_serialout(priv, UART_SETUP_OFFSET, 0x3<<4);
...
static bool up_rxavailable(struct uart_dev_s *dev)
{
  ...
  /* NOTE: In NUCLEI, actual data is also retrieved */
